...
export function ProtectedRoute({ allowedRoles }: ProtectedRouteProps) {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[var(--surface-alt)]">
      ...
      {/* ── Main area: offset by sidebar width on desktop only. On mobile the
             sidebar is an off-canvas drawer (see Sidebar.tsx), so no margin
             should ever be applied below the `md` breakpoint — otherwise the
             page content gets squeezed into a tiny sliver of the screen. ── */}
      <div
        className={cn(
          'flex flex-col flex-1 min-w-0 overflow-hidden transition-all duration-300 ml-0',
      ...
      >
        ...
        <main
          ...
          role="main"
          id="main-content"
        > <div className="w-full max-w-full px-3 py-4 sm:px-6 sm:py-5"> <AnimatePresence mode="wait"> <motion.div
