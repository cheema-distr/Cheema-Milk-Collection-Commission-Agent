    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-16 px-4 md:px-6" dir="ltr">
      ...
      {showEntryModal && activeProfileForEntry && (() => {
        ...
        {showDetailsModal && selectedProfileForDetails && (() => {
          ...
          return (
            <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-end sm:items-center justify-center z-[300] p-0 sm:p-4 animate-fade-in text-left"> <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full max-w-5xl shadow-2xl flex flex-col border border-slate-200 overflow-hidden ledger-modal-panel">
                {/* Header Section */}
                <div className="p-4 sm:p-6 border-b border-slate-200 flex justify-between items-center bg-slate-50/50 flex-shrink-0"> <div className="text-left min-w-0"> <h2 className="text-base sm:text-xl font-black text-slate-800 tracking-tight flex items-center gap-2"> <BarChart2 className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-600 flex-shrink-0" /> <span className="truncate">{profile.customerName} Ledger Sheet</span> </h2> <p className="text-xs text-slate-500 font-medium mt-0.5 truncate">📍 {profile.location} — WhatsApp: {profile.phoneNumber || 'N/A'}</p> </div> <button 
                    onClick={() => setShowDetailsModal(false)}
                    className="p-2 hover:bg-slate-200 rounded-full transition cursor-pointer flex-shrink-0 ml-2"
                  > <X className="w-5 h-5 sm:w-6 sm:h-6 text-slate-400" /> </button> </div>

                {/* Sub-Header / Summary Stats — scrollable on mobile */}
                <div className="border-b border-slate-200 bg-white flex-shrink-0 overflow-x-auto">
                  <div className="p-3 sm:p-6 grid grid-cols-3 sm:grid-cols-3 lg:grid-cols-6 gap-2 sm:gap-3" style={{minWidth: '360px'}}> <div className="p-2 sm:p-3 bg-emerald-50 rounded-xl border border-emerald-100 text-left"> <span className="text-[9px] sm:text-[10px] text-emerald-600 font-bold block mb-1">CURRENT OUTSTANDING</span> <span className="text-sm sm:text-lg font-black text-emerald-900 font-mono">Rs. {fmtAmt(currentBalance)}</span> </div> <div className="p-2 sm:p-3 bg-slate-50 rounded-xl border border-slate-200 text-left"> <span className="text-[9px] sm:text-[10px] text-slate-500 font-bold block mb-1">PERIOD MILK AMOUNT</span> <span className="text-sm sm:text-base font-bold text-slate-800 font-mono">Rs. {fmtAmt(totalMilk)}</span> </div> <div className="p-2 sm:p-3 bg-indigo-50 rounded-xl border border-indigo-100 text-left"> <span className="text-[9px] sm:text-[10px] text-indigo-600 font-bold block mb-1">PERIOD ADVANCES</span> <span className="text-sm sm:text-base font-bold text-indigo-900 font-mono">Rs. {fmtAmt(totalAdvance)}</span> </div> <div className="p-2 sm:p-3 bg-teal-50 rounded-xl border border-teal-100 text-left"> <span className="text-[9px] sm:text-[10px] text-teal-600 font-bold block mb-1">PERIOD CASH PAID</span> <span className="text-sm sm:text-base font-bold text-teal-900 font-mono">Rs. {fmtAmt(totalCash)}</span> </div> <div className="p-2 sm:p-3 bg-amber-50 rounded-xl border border-amber-100 text-left"> <span className="text-[9px] sm:text-[10px] text-amber-605 font-bold block mb-1">PERIOD DISCOUNTS</span> <span className="text-sm sm:text-base font-bold text-amber-900 font-mono">Rs. {fmtAmt(totalDiscount)}</span> </div> <div className="p-2 sm:p-3 bg-rose-50 rounded-xl border border-rose-100 text-left"> <span className="text-[9px] sm:text-[10px] text-rose-600 font-bold block mb-1">PERIOD SPOILED</span> <span className="text-sm sm:text-base font-bold text-rose-900 font-mono">Rs. {fmtAmt(totalSpoiled)}</span> </div> </div>
                </div>
                <div className="px-3 sm:px-6 py-2 sm:py-3 border-b border-slate-200 flex flex-row flex-wrap gap-x-4 gap-y-1 bg-slate-50/50 flex-shrink-0"> <div className="flex flex-col"> <span className="text-[9px] text-slate-500 font-bold uppercase">Total Qty</span> <span className="text-xs sm:text-sm font-black text-slate-800 font-mono">{fmtAmt(totalMilkLiter)}</span> </div> <div className="flex flex-col"> <span className="text-[9px] text-slate-500 font-bold uppercase">Avg Fat</span> <span className="text-xs sm:text-sm font-black text-slate-800 font-mono">{avgFat.toFixed(2)}%</span> </div> <div className="flex flex-col"> <span className="text-[9px] text-slate-500 font-bold uppercase">Avg LR</span> <span className="text-xs sm:text-sm font-black text-slate-800 font-mono">{avgLr.toFixed(2)}</span> </div> <div className="flex flex-col"> <span className="text-[9px] text-slate-500 font-bold uppercase">Avg SNF</span> <span className="text-xs sm:text-sm font-black text-slate-800 font-mono">{avgSnf.toFixed(2)}%</span> </div> <div className="flex flex-col"> <span className="text-[9px] text-slate-500 font-bold uppercase">Avg TS</span> <span className="text-xs sm:text-sm font-black text-slate-800 font-mono">{avgTs.toFixed(2)}</span> </div> </div>

                {/* Filters and Actions Bar */}
                <div className="px-3 sm:px-6 py-3 sm:py-4 border-b border-slate-200 flex flex-col gap-3 bg-slate-50/30 flex-shrink-0"> <div className="flex flex-wrap items-center gap-2"> <Filter className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-slate-400 mr-1 flex-shrink-0" />
                    {[
                      { l: '1D', v: '1D' },
                      { l: '5D', v: '5D' },
                      { l: '7D', v: '7D' },
                      ...
                      <button
                        ...
                        className={`px-2.5 sm:px-3 py-1.5 rounded-lg text-xs font-black transition-all border cursor-pointer ${
                    ...
                    {detailsDateFilter === 'CUSTOM' && (
                  ...
                  </div> <div className="flex items-center gap-2 flex-wrap"> <button
                      onClick={() => {
                        const cols = [
                          { header: 'Date', dataKey: 'date' },
                      ...
                      className="flex items-center gap-1.5 bg-rose-600 hover:bg-rose-700 text-white px-3 sm:px-4 py-2 rounded-xl text-xs font-black transition-all shadow-sm cursor-pointer"
                    > <FileDown className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> <span>Download PDF</span> </button> <button
                      onClick={() => {
                        setShowDetailsModal(false);
                        openSaleEntryDialog(profile, true);
                      }}
                      className="flex items-center gap-1.5 bg-emerald-600 hover:bg-emerald-700 text-white px-3 sm:px-4 py-2 rounded-xl text-xs font-black transition-all shadow-sm cursor-pointer"
