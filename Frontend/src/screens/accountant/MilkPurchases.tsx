...
export default function MilkPurchases() {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* ── Entry table ── */}
      <div className="table-wrapper"> <div className="table-scroll"> <table className="data-table milk-entry-table"><thead><tr><th className="whitespace-nowrap">Name / Party</th> <th className="whitespace-nowrap">Vol (L)</th> <th className="whitespace-nowrap">Fat%</th> <th className="whitespace-nowrap">LR</th> <th className="whitespace-nowrap">SNF%</th> <th className="whitespace-nowrap">TS%</th> <th className="whitespace-nowrap">Total TS</th> <th className="whitespace-nowrap">Rate/TS</th> <th className="whitespace-nowrap">Price/L</th> <th className="whitespace-nowrap">Amount</th> <th></th></tr></thead><tbody>
              {rows.map((row) => (<tr key={row.id}><td> <input type="text" placeholder="Party Name" value={row.name}
                      onChange={e => handleRowChange(row.id,'name',e.target.value)}
                      className="form-input form-input-sm name-input w-full min-w-[130px]" aria-label="Party name" /> </td> <td><input type="number" step="0.01" value={row.vol||''} onChange={e=>handleRowChange(row.id,'vol',e.target.value)} className="form-input form-input-sm w-20" aria-label="Volume" /></td> <td><input type="number" step="0.01" value={row.fat||''} onChange={e=>handleRowChange(row.id,'fat',e.target.value)} className="form-input form-input-sm w-16" aria-label="Fat percentage" /></td> <td><input type="number" step="0.01" value={row.lr||''}  onChange={e=>handleRowChange(row.id,'lr',e.target.value)}  className="form-input form-input-sm w-16" aria-label="LR value" /></td> <td className="whitespace-nowrap"><div className="form-input form-input-sm form-input-readonly font-mono" style={{minWidth:'70px'}}>{row.snf.toFixed(2)}</div></td> <td className="whitespace-nowrap"><div className="form-input form-input-sm form-input-readonly font-mono" style={{minWidth:'70px'}}>{row.tsr.toFixed(2)}</div></td> <td className="whitespace-nowrap"><div className="form-input form-input-sm bg-blue-50 border-blue-200 text-blue-800 font-bold font-mono" style={{minWidth:'80px'}}>{row.totalTs.toFixed(2)}</div></td> <td><input type="number" step="0.01" value={row.rate||''} onChange={e=>handleRowChange(row.id,'rate',e.target.value)} className="form-input form-input-sm w-20" aria-label="Rate per TS" /></td> <td className="whitespace-nowrap"><div className="form-input form-input-sm form-input-readonly font-mono" style={{minWidth:'80px'}}>{row.pricePerLiter.toFixed(2)}</div></td> <td className="whitespace-nowrap"><div className="form-input form-input-sm form-input-readonly font-bold font-mono" style={{minWidth:'90px'}}>{row.amount.toFixed(2)}</div></td> <td> <button onClick={()=>removeRow(row.id)} aria-label="Remove row"
              ...
              ))}</tbody><tfoot><tr><td className="font-bold text-[var(--text-primary)] whitespace-nowrap">TOTAL ({rows.length})</td> <td className="font-bold font-mono whitespace-nowrap">{totals.vol.toFixed(2)}</td> <td className="text-[var(--text-muted)] text-xs whitespace-nowrap">avg {avgFat.toFixed(2)}</td> <td className="text-[var(--text-muted)] text-xs whitespace-nowrap">avg {avgLr.toFixed(2)}</td> <td className="text-[var(--text-muted)] text-xs whitespace-nowrap">avg {avgSnf.toFixed(2)}</td> <td className="text-[var(--text-muted)] text-xs whitespace-nowrap">avg {avgTsr.toFixed(2)}</td> <td className="font-bold text-blue-700 font-mono whitespace-nowrap">{totals.totalTs.toFixed(2)}</td> <td></td> <td className="text-[var(--text-muted)] text-xs whitespace-nowrap">avg {avgPricePerLiter.toFixed(2)}</td> <td className="font-black font-mono text-[var(--text-primary)] whitespace-nowrap">{totals.amount.toFixed(2)}</td> <td></td></tr></tfoot></table> </div> </div>
      ...
      <section> <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-3 mb-3"> <div className="flex items-center gap-3"> <h2 className="text-sm font-black text-[var(--text-primary)] uppercase tracking-wider">Purchase History</h2> <button onClick={downloadPDF} className="btn btn-primary btn-sm gap-1.5"> <Download className="w-3.5 h-3.5"/> PDF
        ...
        <div className="table-wrapper"> <div className="table-scroll"> <table className="data-table milk-history-table"><thead><tr><th>#</th><th>Date</th><th>Name / Party</th> <th>Vol (L)</th><th>TS%</th><th>Total TS</th><th>Amount</th> <th className="text-center">Action</th></tr></thead><tbody>
                {(() => {
