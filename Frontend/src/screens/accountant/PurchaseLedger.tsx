        ...
        {showDetailsModal && selectedProfileForDetails && (() => {
          ...
          return (
            <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-end sm:items-center justify-center z-[300] p-0 sm:p-4 animate-fade-in text-left"> <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full max-w-5xl shadow-2xl flex flex-col border border-slate-200 overflow-hidden ledger-modal-panel">
                {/* Header Section */}
                <div className="p-4 sm:p-6 border-b border-slate-200 flex justify-between items-center bg-slate-50/50 flex-shrink-0"> <div className="text-left min-w-0"> <h2 className="text-base sm:text-xl font-black text-slate-800 tracking-tight flex items-center gap-2"> <BarChart2 className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-600 flex-shrink-0" /> <span className="truncate">{profile.supplierName} Ledger Sheet</span> </h2> <p className="text-xs text-slate-500 font-medium mt-0.5 truncate">📍 {profile.location} — CNIC: {profile.cnicNumber || 'N/A'}</p> </div> <button 
