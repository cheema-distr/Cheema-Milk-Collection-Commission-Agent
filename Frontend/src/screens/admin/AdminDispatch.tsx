    ...
    <div className="space-y-6 max-w-full mx-auto"> <div className="flex justify-between items-center bg-white p-6 rounded-xl border border-slate-200 shadow-sm"> <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-2"> <Truck className="w-6 h-6 text-purple-600" /> Dispatch record
        ...
        > <Plus className="w-4 h-4" /> New dispatch
              ...
              {dispatches.map(d => {
                ...
                return (<tr key={d.id} className="hover:bg-slate-50 transition-colors whitespace-nowrap"><td className="p-4"> <div className="flex flex-row items-center gap-2 flex-wrap"> <p className="font-bold text-slate-800">{fmtDate(d.date)}</p> <p className="text-xs text-slate-500 uppercase tracking-widest">{d.vehicleNumber}</p> </div> </td> <td className="p-4"> <p className="font-bold text-slate-800">{d.roadName || '-'}</p> <p className="text-xs text-slate-500 mt-1">{d.trunkName || '-'}</p> </td> <td className="p-4"> <p className="font-bold text-slate-800">{d.driverName}</p> </td> <td className="p-4"> <div className="flex items-center text-xs w-32"> <strong className="font-mono text-indigo-600 font-bold text-sm bg-indigo-50 px-2.5 py-1 rounded border border-indigo-100">{totalSentLiters} L</strong> </div> </td> <td className="p-4 text-center"> <span className={`inline-block px-2.5 py-1 rounded text-xs font-bold ${
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm"> <div className="p-4 bg-slate-50 border-b border-slate-200 flex flex-col md:flex-row md:items-center justify-between gap-4"> <h3 className="font-bold text-slate-700 flex items-center gap-2"> <FileText className="w-5 h-5 text-indigo-600" /> Dispatch Loss History
            ...
            > <Download className="w-3.5 h-3.5" /> PDF
            </button> </div> </div> <div className="overflow-x-auto"> <table className="w-full text-left border-collapse text-sm dispatch-history-table"><thead><tr className="border-b border-slate-200 bg-slate-50 text-xs uppercase text-slate-500 whitespace-nowrap"><th className="p-3 w-12 text-center">Sr.</th> <th className="p-3 min-w-[90px]">Date</th> <th className="p-3 min-w-[120px]">Vehicle / Driver</th> <th className="p-3 min-w-[160px]">Sent Totals</th> <th className="p-3 border-l border-slate-200 text-emerald-700 bg-emerald-50/30 min-w-[160px]">Received Totals</th> <th className="p-3 border-l border-slate-200 text-red-700 bg-red-50/50 min-w-[140px]">Loss / Variation</th></tr></thead><tbody className="divide-y divide-slate-100">
              {filteredDispatches.filter(d => d.status === 'Received' || d.status === 'Completed').length > 0 ? (
                 filteredDispatches.filter(d => d.status === 'Received' || d.status === 'Completed').map((d, index) => {
                   ...
                   const diffFat = (Number(originalSum.avgFat) - Number(receivedSum.avgFat)).toFixed(2);
                   const tsLoss = (Number(originalSum.totalTsAmount) - Number(receivedSum.totalTsAmount)).toFixed(2);
                   return (<tr key={'hist-' + d.id} className="hover:bg-slate-50 whitespace-nowrap"><td className="p-3 font-mono text-center text-slate-400">{index + 1}</td> <td className="p-3 font-bold text-slate-700">{fmtDate(d.date)}</td> <td className="p-3"> <div className="font-bold text-slate-800 text-xs">{d.vehicleNumber}</div> <div className="text-xs text-slate-500">{d.driverName}</div> </td> <td className="p-3"> <div className="flex flex-row flex-wrap gap-x-3 gap-y-0.5 text-xs"> <span className="text-slate-500 font-medium">Liters: <span className="font-mono font-bold text-indigo-700">{originalSum.totalMilkLiter} L</span></span> <span className="text-slate-500 font-medium">Kgs: <span className="font-mono font-bold">{originalSum.totalMilkKgs} kg</span></span> <span className="text-slate-500 font-medium">Fat: <span className="font-mono font-bold">{originalSum.avgFat}%</span></span> </div> </td> <td className="p-3 border-l border-slate-200 bg-emerald-50/10"> <div className="flex flex-row flex-wrap gap-x-3 gap-y-0.5 text-xs"> <span className="text-emerald-600 font-medium">Liters: <span className="font-mono font-bold text-emerald-700">{receivedSum.totalMilkLiter} L</span></span> <span className="text-emerald-600 font-medium">Kgs: <span className="font-mono font-bold text-emerald-700">{receivedSum.totalMilkKgs} kg</span></span> <span className="text-emerald-600 font-medium">Fat: <span className="font-mono font-bold text-emerald-700">{receivedSum.avgFat}%</span></span> </div> </td> <td className="p-3 border-l border-slate-200 bg-red-50/30"> <div className="flex flex-row flex-wrap gap-x-3 gap-y-0.5 text-xs"> <span className="text-red-500 font-medium">Liters: <span className="font-mono font-bold text-red-600">{literLoss} L</span></span> <span className="text-red-500 font-medium">Kgs: <span className="font-mono font-bold text-red-600">{kgLoss} kg</span></span> <span className="text-red-500 font-medium">Fat: <span className="font-mono font-bold text-red-600">{diffFat}%</span></span> </div> </td></tr>
                   );
                 })
      ...
      {(isAddModalOpen || isReceiveModalOpen) && (() => {
        ...
        return (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-[300] overflow-y-auto"> <div className="bg-white rounded-2xl shadow-xl w-full max-w-7xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 my-auto"> <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-800 text-white"> <h3 className="font-bold flex items-center gap-2">
                ...
                {/* Table Section */}
                <div className="flex-1 min-h-[300px]"> <h4 className="font-bold text-slate-700 mb-2 px-1 text-sm">{isReceive ? 'Review & Receive Locations & Quality Log' : 'dispatch Locations & Quality Log'}</h4> <div className="border border-slate-200 rounded-xl shadow-sm overflow-x-auto bg-white"> <table className="w-full text-xs text-left whitespace-nowrap"><thead className="bg-[#f8f9fa] text-slate-600 border-b border-slate-200"><tr><th className="p-2 border-r border-slate-200 font-bold text-center w-8">Move</th> <th className="p-2 border-r border-slate-200 font-bold w-12">Serial</th> <th className="p-2 border-r border-slate-200 font-bold min-w-[140px]">customer name</th> <th className="p-2 border-r border-slate-200 font-bold min-w-[100px]">Time</th> <th className="p-2 border-r border-slate-200 font-bold min-w-[100px] bg-indigo-50/50">Milk KG</th> <th className="p-2 border-r border-slate-200 font-extrabold min-w-[100px] bg-indigo-100 text-indigo-900">milk litr</th> <th className="p-2 border-r border-slate-200 font-bold min-w-[90px]">Fat %</th> <th className="p-2 border-r border-slate-200 font-bold min-w-[90px]">LR</th> <th className="p-2 border-r border-slate-200 font-bold min-w-[90px]">Temp °C</th> <th className="p-2 border-r border-slate-200 font-bold min-w-[80px] bg-slate-50">SNF %</th> <th className="p-2 border-r border-slate-200 font-bold min-w-[80px] bg-slate-50">TS %</th> <th className="p-2 border-r border-slate-200 font-bold min-w-[90px] bg-slate-50">Total TS</th> <th className="p-2 text-center font-bold">Actions</th></tr></thead><tbody className="divide-y divide-slate-100">
                        {entries.map((ent, idx) => (<tr key={ent.id} className="hover:bg-slate-50"><td className="p-1 border-r border-slate-100 text-center"><span className="text-slate-300 cursor-grab">⋮⋮</span></td> <td className="p-1 border-r border-slate-100 text-center font-mono text-slate-500">{idx + 1}</td> <td className="p-1 border-r border-slate-100"> <input type="text" value={ent.customerName} onChange={e => updateEntry(ent.id, 'customerName', e.target.value)} className="w-full px-2 py-2 border border-slate-200 rounded text-sm focus:ring-1 focus:ring-indigo-500 outline-none" placeholder="Name..." /> </td> <td className="p-1 border-r border-slate-100"> <input type="time" value={ent.time} onChange={e => updateEntry(ent.id, 'time', e.target.value)} className="w-full px-2 py-2 border border-slate-200 rounded text-sm focus:ring-1 focus:ring-indigo-500 outline-none" /> </td> <td className="p-1 border-r border-slate-100 bg-indigo-50/20"> <input type="number" value={ent.milkKg} onChange={e => updateEntry(ent.id, 'milkKg', e.target.value)} className="w-full min-w-[88px] px-2 py-2 border border-slate-200 rounded text-sm focus:ring-1 focus:ring-indigo-500 outline-none font-mono" placeholder="0" /> </td> <td className="p-1 border-r border-slate-100 bg-indigo-50"> <input type="number" value={ent.milkLiter} onChange={e => updateEntry(ent.id, 'milkLiter', e.target.value)} className="w-full min-w-[88px] px-2 py-2 border-2 border-indigo-200 rounded text-sm focus:border-indigo-500 outline-none font-mono font-bold text-indigo-900" placeholder="0" /> </td> <td className="p-1 border-r border-slate-100"> <input type="number" step="0.1" value={ent.fat} onChange={e => updateEntry(ent.id, 'fat', e.target.value)} className="w-full min-w-[78px] px-2 py-2 border border-slate-200 rounded text-sm focus:ring-1 focus:ring-indigo-500 outline-none font-mono" placeholder="0" /> </td> <td className="p-1 border-r border-slate-100"> <input type="number" step="0.1" value={ent.lr} onChange={e => updateEntry(ent.id, 'lr', e.target.value)} className="w-full min-w-[78px] px-2 py-2 border border-slate-200 rounded text-sm focus:ring-1 focus:ring-indigo-500 outline-none font-mono" placeholder="0" /> </td> <td className="p-1 border-r border-slate-100"> <input type="number" step="0.1" value={ent.temp} onChange={e => updateEntry(ent.id, 'temp', e.target.value)} className="w-full min-w-[78px] px-2 py-2 border border-slate-200 rounded text-sm focus:ring-1 focus:ring-indigo-500 outline-none font-mono" placeholder="0" /> </td> <td className="p-2 border-r border-slate-100 bg-slate-50 font-mono text-slate-700 text-right">{ent.snf || '0.00'}</td> <td className="p-2 border-r border-slate-100 bg-slate-50 font-mono text-slate-700 text-right">{ent.ts || '0.00'}</td> <td className="p-2 border-r border-slate-100 bg-slate-50 font-mono text-indigo-700 font-bold text-right">{ent.totalTs || '0.00'}</td> <td className="p-1 text-center"> <button type="button" onClick={() => setEntries(entries.filter(e => e.id !== ent.id))} className="text-slate-400 hover:text-red-600 bg-white hover:bg-red-50 p-1.5 rounded shadow-sm border border-slate-200 transition-colors"> <X className="w-4 h-4" /> </button> </td></tr>
                ...
                {/* Submission actions */}
                <div className="flex flex-col-reverse sm:flex-row sm:justify-end gap-3 pt-4 mt-auto border-t border-slate-100"> <button type="button" onClick={() => isReceive ? setIsReceiveModalOpen(false) : setIsAddModalOpen(false)} className="w-full sm:w-auto px-6 py-2.5 text-sm font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors">
                    Cancel
                  </button>
                  {!isReceive && (
                    <button type="button" onClick={(e) => handleCreateDispatch(e, 'Draft')} className="w-full sm:w-auto px-6 py-2.5 text-sm font-bold text-indigo-600 bg-indigo-50 border border-indigo-200 hover:bg-indigo-100 rounded-xl transition-colors">
                      Save as Draft
                    </button>
                  )}
                  <button type="submit" className="w-full sm:w-auto px-8 py-2.5 text-sm font-bold text-white bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 rounded-xl flex items-center justify-center gap-2 shadow-sm transition-all"> <Save className="w-4 h-4" /> {isReceive ? 'Submit Received Data' : 'Submit disptach'}
