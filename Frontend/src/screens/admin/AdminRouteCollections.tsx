      ...
      {viewCol ? (
        ...
        <>
          ...
          <div className="relative max-w-sm"> <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[var(--text-muted)]" /> <input type="search" placeholder="Search routes or milk testers…" value={searchTerm}
              ...
              className="form-input pl-9" /> </div> <div className="table-wrapper"> <div className="table-scroll"> <table className="data-table"><thead><tr className="whitespace-nowrap"><th className="min-w-[90px]">Date</th><th>Route Name</th><th>Milk Tester / Tanker</th> <th className="text-center">Stops</th><th>Volume (Kg)</th><th>Status</th><th></th></tr></thead><tbody>
                  {filteredCols.length === 0 ? (<tr><td colSpan={7} className="py-12 text-center text-[var(--text-muted)]">No route collections found.</td></tr>
                  ) : filteredCols.map(col => (<tr key={col.id} className="whitespace-nowrap"><td className="font-mono text-xs text-[var(--text-secondary)] whitespace-nowrap min-w-[90px]">{fmtDate(col.date)}</td> <td className="font-bold text-[var(--text-primary)]">{col.routeName}</td> <td> <p className="font-semibold text-[var(--text-primary)]">{col.mtName}</p> <p className="text-[11px] text-[var(--text-muted)] font-mono">{col.tankerNumber}</p> </td> <td className="text-center font-bold">{col.stops.length}</td> <td className="font-bold font-mono text-blue-600">{col.stops.reduce((a,b)=>a+b.milkKgs,0).toFixed(2)} Kg</td> <td><span className={cn('badge',
